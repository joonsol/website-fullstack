...
router.post("/signup", async (req, res) => {
    try {
        ...
        if (existingUser) {
            return res.status(900).json({ message: "이미 존재하는 사용자입니다." })
        }
    ...
    try {
        ...
        if (!user) {
            return res.status(401)
                .json({ message: "사용자를 찾을수 없습니다." })
        }
        if (!user.isActive) {
            return res.status(401)
                .json({ message: "비활성 계정입니다." })
        }
        if (user.isLoggedIn) {
            return res.status(401)
                .json({ message: "이미 다른 기기에서 로그인되었습니다." })
        }
...
router.post("/logout", async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(401).json({ message: "유효하지 않은 토큰입니다. 강제 로그아웃되었습니다." });
        }
...
router.post("/verify-token", (req, res) => {
    ...
    try {
        ...
        return res.status(401).json({ isValid: false, message: "유효하지 않는 토큰입니다." })
    }
})
